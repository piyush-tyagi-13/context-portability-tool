"""VaultMap — manages <vault>/.mdcore-meta.yaml.
...
META_FILENAME = ".mdcore-meta.yaml"
...
class VaultMap:
    ...
    def all_vault_folders(self) -> list[str]:
        ...
        for p in sorted(self._vault_path.rglob("*")):
            if p.is_dir() and p != self._vault_path:
                ...
                # Skip hidden dirs and mdcore-output
                if not any(part.startswith(".") for part in rel.split("/")) and rel != "mdcore-output":
                    folders.append(rel)
        return folders
    ...
    def write_template(self) -> Path:
        """Write (or refresh) .mdcore-meta.yaml with all current folders.
