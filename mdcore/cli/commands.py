...
def _write_init_config(
    ...
    # embed api key / aggregator fields
    if embed_backend == "aggregator":
        embed_api_key_line = f"  aggregator_category: {aggregator_category or 'general_purpose'}"
    else:
...
def init(
    ...
    if backend == "aggregator":
        ...
        console.print("\n[dim]Embedding backend — aggregator can handle embeddings too[/dim]")
        console.print("  [dim]Options: aggregator, ollama, gemini[/dim]")
        embed_backend = typer.prompt("Embedding backend", default="aggregator")
        embed_model = ""
        primary_model = ""
        synth_model = ""
